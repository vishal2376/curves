package com.vishal2376.curves.core.presentation.theme

import androidx.compose.ui.graphics.Color

val LatteBase = Color(0xFFEFF1F5) // background
val LatteSurface = Color(0xFFDCE0E8)
val LatteOverlay = Color(0xFFCCD0DA)
val LatteText = Color(0xFF4C4F69)
val LatteSubtext = Color(0xFF6C6F85)
val LatteLavender = Color(0xFF7287FD)
val LattePink = Color(0xFFEA76CB)
val LatteBlue = Color(0xFF1E66F5)
val LatteTeal = Color(0xFF179299)
val LattePeach = Color(0xFFFE640B)
val LatteYellow = Color(0xFFDF8E1D)
val LatteMauve = Color(0xFF8839EF)


val MochaBase = Color(0xFF1E1E2E) // background
val MochaSurface = Color(0xFF181825)
val MochaOverlay = Color(0xFF313244)
val MochaText = Color(0xFFD9E0EE)
val MochaSubtext = Color(0xFFA6ADC8)
val MochaLavender = Color(0xFFB4BEFE)
val MochaPink = Color(0xFFF5C2E7)
val MochaBlue = Color(0xFF89B4FA)
val MochaTeal = Color(0xFF94E2D5)
val MochaPeach = Color(0xFFFAB387)
val MochaYellow = Color(0xFFF9E2AF)
val MochaMauve = Color(0xFFCBA6F7)

val lavender = Color(0xFFD0BCFF)
val black = Color(0xFF0D0D0D)
val gray = Color(0xFF121212)
val white = Color(0xFFEBEBEB)


val NeonYellow = Color(0xFFFFD600)
val ElectricAmber = Color(0xFFFFC107)
val DeepBlue = Color(0xFF001122)
val DarkBackground = Color(0xFF010A1A)
val WarmOrange = Color(0xFFFF9100)

val SoftLavender = Color(0xFFB18AFF)
val SoftCyan = Color(0xFF6EE7E7)
val SoftBlue = Color(0xFF80AFFF)
val SoftPink = Color(0xFFFFAACC)
val SoftOrange = Color(0xFFFFB86C)
val SoftGreen = Color(0xFF7EFAB9)
val SoftYellow = Color(0xFFFFF59E)
val SoftRed = Color(0xFFFF7F7F)
val SoftMagenta = Color(0xFFEA80FC)
val SoftTeal = Color(0xFF64FFDA)